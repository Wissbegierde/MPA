...
def backMenu(request,nombre):
    Eusuario = Juegos.objects.get(username=nombre)  

    return render(request, "usuarios/menu.html", {"user": Eusuario})   
...
def eliminarUsuario(request,nombre):
    Eusuario = Usuario.objects.get(username=nombre)
    Jusuario = Juegos.objects.get(username=nombre)
...
def editarUsuario(request,nombre):
    ...
    return render(request, "usuarios/actualizar.html", {"user": Eusuario})


def guardarCambios(request,nombre):
    if request.POST["username"] == "":
        Jusuario = Juegos.objects.get(username=nombre)
        return render(request, "usuarios/menu.html", {"user": Jusuario})  
    else:
        Eusuario = Usuario.objects.get(username=nombre)
        Jusuario = Juegos.objects.get(username=nombre)
        #Usuario
        Eusuario.username = request.POST["username"]
        Eusuario.email = request.POST["email"]   
        Eusuario.save() 

        #Juegos
        Jusuario.username = request.POST["username"]
        Jusuario.save()
        return render(request, "usuarios/menu.html", {"user":Jusuario})
